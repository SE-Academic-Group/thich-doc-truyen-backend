...
class TruyenFullTest {
    ...
    @Test
    void testGetChapterDetailFromJsonString_MissingFields() {
        String json = "{ \"data\": { \"story_name\": \"Test Novel\", \"chapter_name\": \"Chapter 1\" }, \"status\": \"success\" }";
        ChapterDetail chapterDetail = truyenFull.getChapterDetailFromJsonString(json);

        assertNotNull(chapterDetail);
        assertEquals("Test Novel", chapterDetail.getNovelTitle());
        assertEquals("Chapter 1", chapterDetail.getTitle());
        assertNull(chapterDetail.getContent());
        assertNull(chapterDetail.getUrl());
    }
    ...
    @Test
    void testBuildSearchUrl_KeywordWithSpaces() {
        String keyword = "example keyword";
        int page = 1;
        String expectedUrl = "https://api.truyenfull.vn/v1/tim-kiem?title=example%20keyword&page=1";
        String actualUrl = truyenFull.buildSearchUrl(keyword, page);
    ...
    void testBuildSearchUrl_NullKeyword() {
        ...
        String expectedMessage = "Cannot invoke \"String.replace(CharSequence, CharSequence)\" because \"keyword\" is null";
